...
class CTReportDatasetinfer(Dataset):
    def __init__(self, data_folder, csv_file, min_slices=20, resize_dim=500, force_num_frames=True, labels="labels.csv", split='train'):
    ...
    def prepare_samples(self):
        ...
        for patient_folder in tqdm.tqdm(patient_folders):
            ...
            for accession_folder in accession_folders:
                if self.split == 'train':
                    filename = os.path.basename(accession_folder).replace("train_", "")
                    npz_file = os.path.join(accession_folder, filename + ".npz")
                else:
                    filename = os.path.basename(accession_folder).replace("test_", "")
                    npz_file = os.path.join(accession_folder, filename + ".npz")
                ...
                text_final = "".join(str(text) if str(text) != "Not given." else "" for text in list(impression_text))

                onehotlabels = test_df[test_df["impression_id"] == accession_number]["one_hot_labels"].values
                if len(onehotlabels) > 0:
                    samples.append((npz_file, text_final, onehotlabels[0]))
